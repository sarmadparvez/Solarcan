...
class AppointmentApi extends SugarApi
{
    ...
    public function getAvailableAppointments(ServiceBase $api, array $args)
    {
        ...
        // Get available appointments by excluding thes appointments/users for whom daily or weekly limit is
        // reached
        $sql = "
            SELECT 
            ...
            FROM
                ...
                users pc_u ON (pc_u.id = rt_postal_codes_users.rt_postal_codes_usersusers_idb)
                    ...
                    INNER JOIN
                meetings m ON (m.created_by = pc_u.id)
    ...
    /**
    * Get user ids for the users whose per week appointment limit is reached
    * @param string $start_time_before
    */
    private function getUsersHavingWeekAppointmentLimitReached($start_time_before = '')
    {
        $start_time_after = $this->getTimeDate()->nowDbDate();
        ...
        $s_query->select()->fieldRaw('m.timeslot_name', 'timeslot_name');
        $s_query->select()->fieldRaw('m.timeslot_datetime', 'timeslot_datetime');
        $s_query->select()->fieldRaw('DATE(m.timeslot_datetime)', 'dateonly');
        $s_query->select()->fieldRaw(
            'DATE_ADD(DATE(m.timeslot_datetime), INTERVAL(-WEEKDAY(DATE(m.timeslot_datetime))) DAY)', 
            'week_of'
        );
        ...
        $s_query->joinTable(
            ...
            ->gte('m.date_start', $start_time_after)
    ...
    */
    ...
    // End DEV-320 : QA Portail : Postal Code: Not found in CRM
        ...
        $query = "  SELECT u.id, m.id as meeting_id, m.status as status
                    ...
                    WHERE u.deleted = 0
                    AND m.deleted = 0
                    AND DATE(m.timeslot_datetime) = '$meeting_date'
        ...
        $query .= " AND u.id IN (SELECT pcu.rt_postal_codes_usersusers_idb
                    ...
                    AND u.id NOT IN (SELECT mt.assigned_user_id
                                     FROM meetings mt
                                     WHERE DATE(mt.timeslot_datetime) = '$meeting_date'
        ...
        while($row = $db->fetchByAssoc($result)) {
            ...
            $users_meetings[$row['id']]['status'] = $row['status'];
        }
        if (!empty($potential_assignees)) {
            ...
            if (!empty($filtered_assignees)) {
                ...
                if ($meeting->id != $users_meetings[reset($filtered_assignees)]['meeting_id'] &&
                        $users_meetings[reset($filtered_assignees)]['status'] == 'disponible'
                    ) {
                ...
                if ($meeting_date == $now_date ||
                    ...
                    $meeting->assigned_user_id = reset($filtered_assignees);
