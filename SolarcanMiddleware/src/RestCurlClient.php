...
class RestCurlClient {
    ...
    public function __construct() {
        ...
        $this->http_options[CURLOPT_HTTP_VERSION] = CURL_HTTP_VERSION_1_0;
        ...
        $this->http_options[CURLOPT_TIMEOUT] = 0;

        // check from config if we need to enable curl requests logging
        global $app_config;
        if ($app_config['enable_curl_logs']) {
            // enable curl logging
            $this->http_options[CURLOPT_VERBOSE] = true;
            $verbose = fopen('logs/curl_logs.txt', 'a+');
            $this->http_options[CURLOPT_STDERR] = $verbose;  
        }
    }
